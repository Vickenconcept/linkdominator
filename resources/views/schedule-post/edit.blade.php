...
<form action="{{route('post.update', ['id' => $post->id])}}" method="post" class="mt-6" enctype="multipart/form-data">
    @csrf
    @method('PUT')
            ...
            <div class="flex justify-between mt-4">
                <label class="block text-sm">
                    <button type="button" id="use-aicontent"
                        class="bg-gray-300 text-gray-700 p-2 rounded-md focus-ring:0"
                        data-tooltip-target="tooltip-generate-aicontent"
                    >
                        Use AI Content
                    </button>
                    <div id="tooltip-generate-aicontent" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip
                        Generate AI content 
                ...
                <label class="block text-sm mt-1">
                    ...
                    <div class="text-gray-700 inline-flex" id="words">{{$post->word_counts}}</div>
                    <input style="display: none;"
                    type="number" id="words-input" name="words"
                    class="block mt-1 text-sm w-[5rem]
 form-textarea 
                    focus:border-indigo-400 focus:outline-none 
                    focus:shadow-outline-indigo
                    value="{{$post->word_counts}}" readonly>
                </label>
            </div>
 ...
 form-input 
                ...
                <div>
                    <div style="display: block;" class="get-aicontent-block">
                        ...
                        <div id="tooltip-generate-aicontent-action" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip
                            Generate content
                            <div class="tooltip-arrow" data-popper-arrow></div>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div id="tooltip-cancel-aicontent-action" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip
                        Cancel
                        <div class="tooltip-arrow" data-popper-arrow></div>
                    </div>
                </div>
        ...
        <div class="col-span-4">
            <div class="">
                ...
                @if($post->image)
                <div class="mx-auto mt-3 mb-3 img-preview">
                    <img src="/{{$post->image}}" alt="" class="w-56 h-40 mx-auto" />

                    <!-- <div class="inline-flex w-full justify-center">
                        <button type="button" 
                        class="inline-flex w-full justify-center rounded-md 
                        bg-indigo-600 px-3 py-2 text-sm font-semibold text-white 
                        shadow-sm hover:bg-indigo-500 sm:w-auto mt-2">
                            <span>Remove</span>
                        </button>
                    </div> -->
                </div>
 ...
 rounded-md"
                id="publish_status"
                name="save_mode">
                    <option value="instant" {{  $post->save_mode == 'instant' ? 'selected':'' }}>Instant</option>
                    <option value="schedule" {{ $post->save_mode == 'schedule' ? 'selected':'' }}>Schedule</option>
                    <option value="draft" {{ $post->save_mode == 'draft' ? 'selected':'' }}>Draft</option>
                </select>
            </label>
...
<script>
    ...
    if($('#publish_status').val() == 'schedule')
        $('.schedule_time_label').show()
    else
        $('.schedule_time_label').hide()
...
$('#image').change(function(ev) {
    if(!$('#article_link').val()) {
        ...
        // reader.onload = () => {
        //     $('.img-preview2').attr('src', reader.result);
        //     $('.img-preview2').show()
        // }
